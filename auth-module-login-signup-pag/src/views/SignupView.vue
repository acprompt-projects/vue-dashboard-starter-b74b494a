<template>
  <div class="auth-page">
    <div class="auth-card">
      ...
      <p class="subtitle">Get started with your free dashboard account.</p>
...
function validateName() {
  errors.name = ''
  if (!form.name) { errors.name = 'Name is required'; return false }
  if (form.name.length < 2) { errors.name = 'Must be at least 2 characters'; return false }
  return true
}
...
function validatePassword() {
  ...
  if (form.password.length < 8) { errors.password = 'Must be at least 8 characters'; return false }
  return true
}

function validateConfirm() {
  errors.confirm = ''
  if (!form.confirm) { errors.confirm = 'Please confirm your password'; return false }
  if (form.confirm !== form.password) { errors.confirm = 'Passwords do not match'; return false }
  return true
}
...
async function handleSignup() {
  ...
  const ok = await auth.signup(form.name, form.email, form.password)
